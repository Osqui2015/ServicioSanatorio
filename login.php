...
if (!empty($_SESSION['active'])) {
	...
	if (!empty($_POST)) {
		if (empty($_POST['user']) || empty($_POST['pass'])) {
			...
			$user = mysqli_real_escape_string($conServicios, $_POST['user']);
			$pass = md5(mysqli_real_escape_string($conServicios, $_POST['pass']));
			$query = mysqli_query($conServicios, "SELECT * FROM usuario WHERE usuario= '$user' AND contra = '$pass'");
			...
			if ($result > 0) {
				...
				$_SESSION['NombreApe'] = $data['NombreApe'];
				$_SESSION['Sector']  = $data['Sector'];
